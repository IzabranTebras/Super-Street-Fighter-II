...
ModulePlayerBlanka1::ModulePlayerBlanka1(bool start_enabled) : Module(start_enabled)
{
...
ModulePlayerBlanka1::~ModulePlayerBlanka1()
{
}

// Load assets
bool ModulePlayerBlanka1::Start()
{
	LOG("Loading player");
	...
	typeAnim = LOOP;
	time = clock();
	bodyCollider = App->collision->AddCollider({ 0, 0, 30, 75 }, COLLIDER_PLAYER1, this, true);
	armCollider = App->collision->AddCollider({ 0, 0, 20, 20 }, COLLIDER_PLAYER1, this, false, true);
	legCollider = App->collision->AddCollider({ 0, 0, 20, 20 }, COLLIDER_PLAYER1, this, false, false, true);
	return true;
}

// Unload assets
bool ModulePlayerBlanka1::CleanUp()
{
...
// Update
update_status ModulePlayerBlanka1::Update()
{
	...
	if (clock() - time < 6000){
		...
		case LOOP:
			...
			if (App->input->GetKey(SDL_SCANCODE_W) == KEY_DOWN){
				...
				if (App->input->GetKey(SDL_SCANCODE_A) == KEY_REPEAT){
					punchAttacks = 0;
					if (abs(position.x - App->player2->position.x) <= 20){
						if ((App->input->GetKey(SDL_SCANCODE_D) == KEY_DOWN) && ((App->input->GetKey(SDL_SCANCODE_K) == KEY_DOWN) || (App->input->GetKey(SDL_SCANCODE_I) == KEY_DOWN) || (App->input->GetKey(SDL_SCANCODE_U) == KEY_DOWN)) && counterBack > 10){
							loopAnim = false;
							typeAnim = SINGLE;
							move = &RollAttack;
							counterBack = 0;
						...
						if (App->input->GetKey(SDL_SCANCODE_K) == KEY_DOWN){
							...
							if (App->input->GetKey(SDL_SCANCODE_I) == KEY_DOWN){
								...
								if ((App->input->GetKey(SDL_SCANCODE_L) == KEY_DOWN) || (App->input->GetKey(SDL_SCANCODE_O) == KEY_DOWN)){
									damage = LOW_DAMAGE;
									loopAnim = false;
									typeAnim = SINGLE;
									move = &FLMKick;
					...
					if ((position.x - 1 > 65) && (abs((position.x - App->player2->position.x - 40)*SCREEN_SIZE) < App->renderer->camera.w) && (move == &walk)) {
						if (flip == SDL_FLIP_HORIZONTAL){
							if (position.x - 15 > App->player2->position.x + 15){
								--position.x;
							}
						}
						else{
					...
					if (App->input->GetKey(SDL_SCANCODE_S) == KEY_REPEAT){
						...
						if (App->input->GetKey(SDL_SCANCODE_K) == KEY_DOWN){
							...
							if ((App->input->GetKey(SDL_SCANCODE_I) == KEY_DOWN) || (App->input->GetKey(SDL_SCANCODE_U) == KEY_DOWN)){
								damage = MEDIUM_DAMAGE;
								move = &CMHPunch;
								typeAnim = SINGLE;
								loopAnim = false;
						...
						if (App->input->GetKey(SDL_SCANCODE_S) == KEY_UP){
							...
							if (App->input->GetKey(SDL_SCANCODE_D) == KEY_REPEAT){
								...
								if ((position.x + 1 < 480) && (abs((position.x - App->player2->position.x + 50)*SCREEN_SIZE) < App->renderer->camera.w) && (move == &walk)) {
									if (flip == SDL_FLIP_NONE){
										if (position.x + 15 < App->player2->position.x - 15){
											++position.x;
										}
		...
		case JUMPING:
			...
			if (jumpAttack == false){
				if ((App->input->GetKey(SDL_SCANCODE_K) == KEY_DOWN) || (App->input->GetKey(SDL_SCANCODE_I) == KEY_DOWN) || (App->input->GetKey(SDL_SCANCODE_U) == KEY_DOWN)){
					jumpAttack = true;
					loopAnim = false;
					if ((abs(position.x - App->player2->position.x) <= 20) && (abs(position.y - App->player2->position.y) <= 80)){
						damage = HIGH_DAMAGE;
						move = &FJPunch;
					}
					else{
					...
					if ((App->input->GetKey(SDL_SCANCODE_L) == KEY_DOWN) || (App->input->GetKey(SDL_SCANCODE_O))){
						damage = MEDIUM_DAMAGE;
						jumpAttack = true;
						loopAnim = false;
						move = &JLMKick;
			...
			if (jumpMove.x != 646){
				...
				if ((jumpDirection != 0) && ((position.x + 1 < 480) && (position.x - 1 > 65)) && (abs((position.x - App->player2->position.x - 40)*SCREEN_SIZE) < App->renderer->camera.w) && (abs((position.x - App->player2->position.x + 50)*SCREEN_SIZE) < App->renderer->camera.w)) {
		...
		case ELECTRIC:
			if (counter > 50){			// change to timer
				if ((App->input->GetKey(SDL_SCANCODE_K) != KEY_DOWN) && (App->input->GetKey(SDL_SCANCODE_I) != KEY_DOWN)){
					typeAnim = LOOP;
				}
				counter = 0;
			}
...
void ModulePlayerBlanka1::OnCollision(const Collider* colliderA, const Collider* colliderB){
	if (colliderB->type == PLAYER2){
		if (damage == NONE){
			if (App->player2->damage != NONE){
				if (block == false){
					if (move == &jump){
						position.y = 110;
					}
					if (App->player2->damage == LOW_DAMAGE){
						if ((App->player2->move == &CLPunch) || (App->player2->move == &CLMKick)){
							life = life - 12;
						}
						else{
							life = life - 14;
						...
						if (App->player2->damage == MEDIUM_DAMAGE){
							if (App->player2->move == &FMPunch){
								life = life - 18;
							}
							else{
								if (App->player2->move == &MPunch){
									life = life - 20;
								}
								else{
									if (App->player2->move == &JLMKick){
										life = life - 24;
									}
									else{
										life = life - 26;
							...
							if (App->player2->damage == HIGH_DAMAGE){
								if ((App->player2->move == &FJPunch) || (App->player2->move == &JHKick)){
									life = life - 30;
								}
								else{
									life = life - 28;
		...
		if ((life < 1) && (move != &KO)){
			CombatEnd(false);
			App->player2->CombatEnd(false);
			App->audio->PlayFx(fxLose);
		}
	}
}

void ModulePlayerBlanka1::CombatEnd(const bool time){
