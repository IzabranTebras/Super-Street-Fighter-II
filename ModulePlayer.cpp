...
ModulePlayer::ModulePlayer(int player, bool start_enabled) : Module(start_enabled)
{
	...
	if (numPlayer == 1) 
	{
		file.open("InputPlayer1.txt");
	}
	else
	{
		file.open("InputPlayer2.txt");
	}

	// Assignation of inputs variables from substrings of each line
	getline(file, line);
	size_t pos = line.find("> ");
	up = atoi(line.substr(pos+1).c_str());
	getline(file, line);
	pos = line.find("> ");
	down = atoi(line.substr(pos + 1).c_str());
	getline(file, line);
	pos = line.find("> ");
	left = atoi(line.substr(pos + 1).c_str());
	getline(file, line);
	pos = line.find("> ");
	right = atoi(line.substr(pos + 1).c_str());
	getline(file, line);
	pos = line.find("> ");
	lowPunch = atoi(line.substr(pos + 1).c_str());
	getline(file, line);
	pos = line.find("> ");
	mediumPunch = atoi(line.substr(pos + 1).c_str());
	getline(file, line);
	pos = line.find("> ");
	highPunch = atoi(line.substr(pos + 1).c_str());
	getline(file, line);
	pos = line.find("> ");
	lowKick = atoi(line.substr(pos + 1).c_str());
	getline(file, line);
	pos = line.find("> ");
	mediumKick = atoi(line.substr(pos + 1).c_str());
	getline(file, line);
	pos = line.find("> ");
